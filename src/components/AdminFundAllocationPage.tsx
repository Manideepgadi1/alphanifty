...
interface AdminFundAllocationPageProps {
  navigateTo: (page: any, data?: any) => void;
  user: User | null;
  basketData?: any;
}

export function AdminFundAllocationPage({ navigateTo, user, basketData }: AdminFundAllocationPageProps) {
  ...
  // Pre-fill allocations if in edit mode
  const getInitialAllocations = () => {
    if (isEditMode && editingBasket?.funds) {
      // Map existing allocations from the basket
      const existingAllocations: { [key: string]: number } = {};
      editingBasket.funds.forEach((fund: any) => {
        existingAllocations[fund.id] = fund.allocation || 0;
      });
      return existingAllocations;
    } else {
      // Equal distribution for new baskets
      return selectedFunds.reduce((acc: any, fund: any) => {
  ...
  const [allocations, setAllocations] = useState<{ [key: string]: number }>(getInitialAllocations());
  ...
  const handleNext = () => {
    ...
    navigateTo('admin-basket-settings', {
      ...basketData,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <button
          onClick={() => navigateTo('admin-fund-selection', basketData)}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="mb-6">
                ...
                <p className="text-gray-600">
                  Basket: <span className="text-[#2E89C4]">{basketData?.basketName || editingBasket?.name}</span>
                </p>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-[#3BAF4A]">Step 3 of 5</span>
                  <span className="text-sm text-gray-500">Fund Allocation</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-gradient-to-r from-[#3BAF4A] to-[#2E89C4] h-2 rounded-full" style={{ width: '60%' }} />
                </div>
              </div>
                ...
                {selectedFunds.map((fund: any, index: number) => (
                  <div key={fund.id} className="border-2 border-gray-200 rounded-lg p-4 hover:border-[#2E89C4]/50 transition-colors">
                    ...
                    <div className="grid grid-cols-4 gap-4 mt-3 pt-3 border-t border-gray-100">
                      <div>
                        <p className="text-xs text-gray-500">1Y CAGR</p>
                        <p className="text-sm text-[#3BAF4A]">{fund.cagr1Y}%</p>
                      </div>
                      <div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-lg p-6 sticky top-24">
              ...
              <div className="space-y-2 mb-6 max-h-80 overflow-y-auto">
                ...
                {selectedFunds.map((fund: any) => (
                  <div key={fund.id} className="flex items-center justify-between text-sm">
                    <span className="text-gray-700 truncate flex-1 mr-2" title={fund.name}>
                      {fund.name}
                    </span>
              ...
              <div className="space-y-3 pt-4 border-t border-gray-200">
                ...
                >
                  <span>Next: Basket Settings</span>
